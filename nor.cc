...
std::array<d_type, len> func(const std::array<d_type, len> s, const bool _x, const bool _y) {
    ...
    const std::array<d_type, len> s3 = {
        (s2[I[0]] & x) | (s2[C_inv[0]] & ~x),
        (s2[I[1]] & x) | (s2[C_inv[1]] & ~x),
        (s2[I[2]] & x) | (s2[C_inv[2]] & ~x),
        (s2[I[3]] & x) | (s2[C_inv[3]] & ~x),
        (s2[I[4]] & x) | (s2[C_inv[4]] & ~x)
    };

    const std::array<d_type, len> s4 = {
        (s3[I[0]] & y) | (s3[B_inv[0]] & ~y),
        (s3[I[1]] & y) | (s3[B_inv[1]] & ~y),
        (s3[I[2]] & y) | (s3[B_inv[2]] & ~y),
        (s3[I[3]] & y) | (s3[B_inv[3]] & ~y),
        (s3[I[4]] & y) | (s3[B_inv[4]] & ~y)
    };
